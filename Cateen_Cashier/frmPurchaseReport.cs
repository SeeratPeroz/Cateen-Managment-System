     ...
     public partial class frmPurchaseReport : Form
    {
        ...
        void showOnStockProducts(string Search, string fromDate, string toDate)
        {
            try
            {
                ...
                {
                    AD.SelectCommand = new SqlCommand("SELECT *  FROM [Canteen_Database].[dbo].[vw_PurchaseReport]", DBContext.con);

                }
                else if (Search != null & fromDate == null & toDate == null)
                {
                    AD.SelectCommand = new SqlCommand("SELECT *  FROM [Canteen_Database].[dbo].[vw_PurchaseReport] WHERE [Invoice #] LIKE '%" + Search + "%' OR [Supplier] LIKE '%" + Search + "%'", DBContext.con);

                }
                else if (Search != null & fromDate != null & toDate != null)
                {
                    AD.SelectCommand = new SqlCommand("SELECT *  FROM [Canteen_Database].[dbo].[vw_PurchaseReport] where([Purchase Date] < '" + toDate + "' and[Purchase Date] >= '" + fromDate + "') and([Invoice #] LIKE '%" + Search + "%' OR [Supplier] LIKE '%" + Search + "%')  ", DBContext.con);

                }
                else
                {
                    AD.SelectCommand = new SqlCommand("SELECT *  FROM [Canteen_Database].[dbo].[vw_PurchaseReport] where([Purchase Date] < '" + toDate + "' and [Purchase Date] >= '" + fromDate + "') ", DBContext.con);
                }
        ...
        private void btn_Export_to_exel_Click_1(object sender, EventArgs e)
        {
            export_excel();
        }
